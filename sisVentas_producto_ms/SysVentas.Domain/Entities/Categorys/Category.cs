...
namespace SysVentas.Domain.Entities.Categorys
{
    public class Category: Entity<long>
    {
        ...
        public DomainValidation CanAddProduct(string code,decimal amount) {
            var validator = new DomainValidation();
        ...
        public Product AddProduct(DateTime date,string code, string name, decimal amount,decimal price) {
        ...
        public DomainValidation CanUpdateStockProduct(long idProduct,decimal amount) {
            ...
            if (Status == StatusView.Get(StatusObject.Inactive)) {
                validator.AddFailed("Actualizar stock a un Producto", $"No se puede actualizar el Stock del producto porque la categoría {Name} está inactiva ");
                return validator;
            }
            var product = GetProductForId(idProduct);            
            if (product is null)
        ...
        public void UpdateStockProduct(long idProduct, decimal amount)
        {
